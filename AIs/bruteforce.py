...
def bruteforce(remaining, vertex, path, weight, trajet,player1_location):
    global best
    global best_path
    ...
    if not remaining:
        ...
        for v in remaining:
            #completly ridiculous fix to avoid key errors with (0,0) #1
            #we delete the vertex from remaining
                ...
                bruteforce(remaining_withoutv, v, path+[v], weight+ trajet[vertex][v][1], trajet,player1_location)
...
def preprocessing(maze, width, height, player1_location, player2_location, pieces_of_cheese, turn_time):
 ...
 #   global chemin
    ...
    bruteforce(pieces_of_cheese,player1_location,[],0,trajet,player1_location)
    print(best_path)
    really_best_path=list(reversed(best_path))
    really_best_path.append(player1_location)
    print(really_best_path)
